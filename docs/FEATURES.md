...
- **New Entry form**
  ...
  - Website name/link, reference resources, and attached documents per article.
...
- **Project Folders & quick upload** — drop many files into a project folder; each becomes a review-status entry. Uploads **stream to disk** (no RAM buffering), run **3 at a time with live progress bars**, reject byte-identical duplicates, and accept files up to **500 MB** (5 GB per user per day).
...
- **Data Entry Tasks** — every entry in one table: search across content/website/agent, filter by department, status and date range, **inline status change**, side-panel review with full article, custom values, resources and **document download**, delete.
...
- **Performance**: query pooling and auth caching, streaming multipart uploads, OCR concurrency gate, upload quotas.
